...
def main(args):
    ...
    if args.eval:
        test_stats, coco_evaluator, results = evaluate(model, criterion, postprocessors,
                                              data_loader_val, base_ds, device, args.output_dir)
        if args.output_dir:
            utils.save_on_master(coco_evaluator.coco_eval["bbox"].eval, output_dir / "eval.pth")
            utils.save_on_master(results, output_dir / "prediction.pth")
            # torch.save(results, output_dir / f"prediction_{utils.get_rank()}.pth")
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % args.eval_epoch == 0 or epoch == (args.lr_drop - 1) or epoch == (args.epochs - 1):
            test_stats, coco_evaluator, results = evaluate(
